...
namespace adore
{
    namespace if_ROS
    {
        ...
        void init(int argc, char **argv, double rate, std::string nodename)
        {
            ...
            Baseapp::initSim();     
            prediction_publisher = getRosNodeHandle()->advertise<adore_if_ros_msg::OccupancyCylinderPredictionSet>("ENV/Prediction/expected_raw",1);
        ...
        void receive_mcm(mcm_dmove_mcm_dmove::MCM msg)
        {
            ...
            {
                std::cout<<"\n"<<v2xStationID<<"\t"<<msg.header.stationID.value;
            ...
            if((state_reader_)!=0 && state_reader_->hasData() )
            {
                ...
                if(msg_vm.maneuverContainer.vehicleManeuver.plannedTrajectory.elements.size()>0)
                {
                    ...
                    std::cout<<"\n"<<X<<"\t"<<Y<<"\t"<<msg_vm.maneuverContainer.vehicleManeuver.plannedTrajectory.elements.size();
